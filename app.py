...
# 🎁 盲盒專區：官方內建之 $150+ 等值獎品池
INTERNAL_BLINDBOX_POOL = [
    {"name": "【全新】威秀影城全台灣通用電影交換券 🎬", "tag": "新品"},
    {"name": "【星巴克】$165 星巴克隨行飲料電子即享券 ☕", "tag": "新品"},
    {"name": "【小米】Xiaomi 22.5W 行動電源 10000 ⚡", "tag": "新品"},
    {"name": "【全新】Logitech 羅技 B170 無線滑鼠 🖱️", "tag": "新品"},
    {"name": "【生活好物】純白極簡超大容量不鏽鋼保溫冰霸杯 🥤", "tag": "新品"},
    {"name": "【麥當勞】超值全餐豪飽雙人電子兌換券 🍔", "tag": "新品"}
]
...
# ==========================================
    ...
    # ------------------------------------------
    # 功能 5: 盲盒專區 (內建全新商品 + 刷卡直接派發虛擬券序號)
    # ------------------------------------------
    elif st.session_state.current_menu == "盲盒專區":
        st.subheader("🎁 幸運官方盲盒抽獎池 ($150 / 次)")
        st.write(
            "每次購買只需支付 **$150 元信用卡手續費**，即可隨機獲得平台官方準備、**價值大於或等於 $150 元** 的全新實體好物或精選即享券憑證！")

        st.markdown("#### 🛍️ 官方精選內建盲盒部分好物預覽：")
        col_preview = st.columns(3)
        for idx, item in enumerate(INTERNAL_BLINDBOX_POOL[:3]):
            with col_preview[idx]:
                st.markdown(f"""
                <div style="background:#ffffff; padding:15px; border-radius:12px; border:1px solid #dee2e6; text-align:center;">
                    <span style="background:#e64980; color:white; padding:2px 8px; border-radius:10px; font-size:11px;">{item['tag']}</span>
        ...
        if pay_valid:
            if st.button("🔒 刷卡支付 $150 並開啟盲盒 🎰", type="primary", use_container_width=True):
                ...
                # 直接插入到優惠券資料庫中，歸類為使用者的未使用優惠券項目
                conn = sqlite3.connect(DB_NAME)
                conn.execute(
                    "INSERT INTO vouchers (student_id, gift_name, code, timestamp, status) VALUES (?, ?, ?, '2026-06-03', '未使用')",
                    (current_student, f"【盲盒大獎】{won_gift['name']}", v_code))
                conn.commit()
                conn.close()
            ...
            st.button("🔒 請先於上方正確填寫信用卡付款資訊以解鎖盲盒購買", disabled=True, use_container_width=True)